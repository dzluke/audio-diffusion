...
class LukeUnet(nn.Module, ModelMixin):
    def __init__(self, in_dim, in_ch, out_ch,
        ...
        # Normalize in_dim to tuple (W, H)
        if isinstance(in_dim, int):
            in_dim = (in_dim, in_dim)

        self.ch = ch
        self.in_dim = in_dim  # (W, H)
        self.num_resolutions = len(ch_mult)
        self.num_res_blocks = num_res_blocks
        self.input_dims = (in_ch, in_dim[1], in_dim[0])  # (C, H, W)
    ...
    def forward(self, x, sigma, cond=None):
        assert x.shape[3] == self.in_dim[0] and x.shape[2] == self.in_dim[1], \
            f'Input spatial dims {(x.shape[3], x.shape[2])} must match in_dim {self.in_dim}'
